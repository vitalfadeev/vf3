...
Font_Size {
    ...
    ID {
        ...
        Font_Glyph.ID
        open (dchar index) {
            return Font_Glyph.open (ftface,index);
        }
    }
}
...
Font_Glyph {
    static
    ID
    open (FT_Face ftface, dchar index) {
        return ID (ftface,index);
    }
    ...
    ID {
        ...
        this (FT_Face ftface, dchar index) {
            this.index = index;
            this.ftface = ftface;
...
Glyph_to_points (E) {
    ...
    move_to_cb (const FT_Vector* to, void* _self) {
        ...
        self.s.back.points ~= E.Point (cast (int) to.x/64,cast (int) to.y/64);
...
unittest {
    ...
    auto r = Font.open (pathname).open (size).open ('A').read ();
